    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        holder.linear.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                int p = 0;
                if (CurrencyActivity.flag == 1) {
                    if (holder.currency.getText().toString().toLowerCase().equals(CurrencyActivity.countryTo.getText().toString().toLowerCase())) {
                        p = 1;
                    } else {
                        CurrencyActivity.countryFrom.setText(holder.currency.getText().toString());
                    }
                } else if (CurrencyActivity.flag == 2) {
                    if (holder.currency.getText().toString().toLowerCase().equals(CurrencyActivity.countryFrom.getText().toString().toLowerCase())) {
                        p = 1;
                    } else {
                        CurrencyActivity.countryTo.setText(holder.currency.getText().toString());
                    }
                }
                if (p == 0) {
                    CurrencyActivity.behavior.setState(BottomSheetBehavior.STATE_COLLAPSED);
                    CurrencyActivity.behavior.setPeekHeight(0);
                    CurrencyActivity.currency_to.setText("");
                    CurrencyActivity.currency_from.setText("");
                    CurrencyActivity.search.setText("");
                    InputMethodManager imm = (InputMethodManager) context.getSystemService(Context.INPUT_METHOD_SERVICE);
                    imm.hideSoftInputFromWindow(holder.linear.getWindowToken(), 0);
                    Prefs.setPrefs("country_from", CurrencyActivity.countryFrom.getText().toString(), context);
                    Prefs.setPrefs("country_to", CurrencyActivity.countryTo.getText().toString(), context);
                } else {
